...
export default function LeftScrollBand({ 
  children, 
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const startPosition = containerWidth * startOffset;
      // End position: stop when text's left edge reaches left margin with some padding for readability
      const endPosition = 20; // Small padding so text doesn't touch the edge
      
      // Calculate current position based on scroll
      const currentPosition = startPosition - (scrollY * speed);
      
      // Clamp the position to not go past the left margin
      const clampedPosition = Math.max(currentPosition, endPosition);
